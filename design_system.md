...
- **Primary Headings (H1, H2, H3):** `Fraunces`, Serif
  - _Weights:_ 500 (Medium), 600 (Semi-Bold)
  - _Usage:_ Page titles, main numbers in KPI cards
- **Body & Interface Text:** `Inclusive Sans`, Sans-Serif (or fallback `IBM Plex Sans` / `-apple-system`)
  - _Weights:_ 400 (Regular), 500 (Medium), 700 (Bold)
  - _Usage:_ Table contents, labels, navigation, reports
- **Monospace (Data / ID values):** `JetBrains Mono` or `Courier New`
  - _Usage:_ Employee IDs (`EMP-00021`), currency figures, salary numbers
